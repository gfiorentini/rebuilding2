...
$aTIPOFONDO=array(1=>"Regionale",2=>"Statale",3=>"FSE",4=>"Misto","Sanitario",9=>"Europeo");
...
<style>
  ...
  <body class="bg-light">
    ...
    <?php 
		...
		$aBREADCUMB[0]["titolo_pagina"]="Home Page";
    ...
    <section class="py-6 bg-light">
      <div class="col-10 offset-1">
        <div class="row">
          ...
          <div class="col-6">    
              <p align="right">
                  <button type="button" class="btn btn-primary-soft btn-xs" onclick="apriRENDICONTAZIONE('0')" <?php if(!$operatore_flagamministratore || !empty($operatore_flagdirigente)) echo 'style="display:none"'; ?>><i class="fe fe-plus"></i>&nbsp;Flusso finanziario</button>
                  <button type="button" class="btn btn-sm btn-primary-soft  btn-xs" onclick="xlsDATA();"><i class="fe fe-list"></i>&nbsp;Esporta in excel</button>
        ...
        <div class="row">
          <div class="col-12">
            ...
            <form class="rounded shadow" method="post" action="rendicontazione" id="formRENDICONTAZIONE" name="formRENDICONTAZIONE" >
              ...
              <div class="input-group input-group-lg p-2" style="background-color: #FFFFFF; <?php if(!$operatore_flagamministratore) echo 'display:none'; ?> ">
                ...
                <div class="form-floating col-sm-4 p-2">
                  <select id="rendicontazione_ente" name="rendicontazione_ente" class="form-select" required>
                    <option value='0'>Tutti</option>
                    <?php
              ...
              <table class="table " id="tblREPORT" name="tblREPORT">
                <thead class="fs-6">
                  <tr>
                    ...
                    <th scope="col" style="width: 35%">DGR riferimento</th>
                    <th scope="col" style="width: 30%">Titolo</th>                    
                ...
                <tbody id="post_data" class="fs-6">
                  <?php
                    ...
                    foreach ($aFLUSSI as $key => $aDATI) 
                    {
                      ...
                      $flussofinanziario_documento1=$aDATI["flussofinanziario_documento1"];
                      $flussofinanziario_documento2=$aDATI["flussofinanziario_documento2"];
                      $flussofinanziario_documento3=$aDATI["flussofinanziario_documento3"];
                      $flussofinanziario_documento4=$aDATI["flussofinanziario_documento4"];
                      $flussofinanziario_documento5=$aDATI["flussofinanziario_documento5"];
                      $flussofinanziario_documento6=$aDATI["flussofinanziario_documento6"];
                      $flussofinanziario_tipodocumento1=$aDATI["flussofinanziario_tipodocumento1"];
                      $flussofinanziario_tipodocumento2=$aDATI["flussofinanziario_tipodocumento2"];
                      $flussofinanziario_tipodocumento3=$aDATI["flussofinanziario_tipodocumento3"];
                      $flussofinanziario_tipodocumento4=$aDATI["flussofinanziario_tipodocumento4"];
                      $flussofinanziario_tipodocumento5=$aDATI["flussofinanziario_tipodocumento5"];
                      $flussofinanziario_tipodocumento6=$aDATI["flussofinanziario_tipodocumento6"];
                      ...
                      $confirm='<a href=\'#\'><span class=\'badge bg-primary-soft\'>NO</span></a>&nbsp;<a href=\'rebuilding_rendicontazione.php?_RENDICONTAZIONE='.$aDATI["idrebuilding_flussofinanziario"].'&_elimina=true\'><span class=\'badge bg-primary-soft\' >SI</span></a>';
                      ...
                      if($operatore_flagamministratore==1 && empty($operatore_flagdirigente))
                      {

                        if($flussofinanziario_stato==2)
                        {
                            echo '<button id="editassegnazione" name="editassegnazione" class="btn btn-xs btn-rounded-circle btn-success" onclick="apriRENDICONTAZIONE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-check"></i></button>
                              <button id="printassegnazione" name="printassegnazione" class="btn btn-xs btn-rounded-circle btn-success" onclick="printRENDICONTAZIONE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-printer"></i></button>
                              <button id="viewscadenze" name="viewscadenze" class="btn btn-xs btn-rounded-circle btn-primary" onclick="openSCADENZE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-clock"></i></button>                                                            
                              <button id="deleteassegnazione'.$aDATI["idrebuilding_flussofinanziario"].'" name="deleteassegnazione'.$aDATI["idrebuilding_flussofinanziario"].'" class="btn btn-xs btn-rounded-circle btn-danger" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="left" title="Confermi l\'eliminazione?" data-bs-content="'.$confirm.'" data-bs-html=true data-bs-trigger="focus"><i class="fe fe-x"></i></button>';
                        }
                        else      
                        {
                            echo '<button id="editassegnazione" name="editassegnazione" class="btn btn-xs btn-rounded-circle btn-primary" onclick="apriRENDICONTAZIONE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-check"></i></button>
                              <button id="printassegnazione" name="printassegnazione" class="btn btn-xs btn-rounded-circle btn-primary" onclick="printRENDICONTAZIONE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-printer"></i></button>
                              <button id="viewscadenze" name="viewscadenze" class="btn btn-xs btn-rounded-circle btn-primary" onclick="openSCADENZE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-clock"></i></button>                              
                              <button id="deleteassegnazione'.$aDATI["idrebuilding_flussofinanziario"].'" name="deleteassegnazione'.$aDATI["idrebuilding_flussofinanziario"].'" class="btn btn-xs btn-rounded-circle btn-danger" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="left" title="Confermi l\'eliminazione?" data-bs-content="'.$confirm.'" data-bs-html=true data-bs-trigger="focus"><i class="fe fe-x"></i></button>';                          
                        }
                      ...
                      {
                        if($flussofinanziario_stato==2 || !empty($operatore_flagdirigente))
                            echo '<button id="editassegnazione" name="editassegnazione" class="btn btn-xs btn-rounded-circle btn-success" onclick="apriRENDICONTAZIONE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-check"></i></button>
                                 <button id="printassegnazione" name="printassegnazione" class="btn btn-xs btn-rounded-circle btn-primary" onclick="printRENDICONTAZIONE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-printer"></i></button>
                                 <button id="viewscadenze" name="viewscadenze" class="btn btn-xs btn-rounded-circle btn-primary" onclick="openSCADENZE(\''.$idrebuilding_flussofinanziario.'\')" ><i class="fe fe-clock"></i></button> ';
...
function apriRENDICONTAZIONE(myFLUSSO)
{
  window.location.href='flussofinanziario?_RENDICONTAZIONE='+myFLUSSO
}
...
function xlsDATA()
{
  myTARGET=$("#rendicontazione_areaintervento").val()
  myANNO=$("#rendicontazione_anno").val()
  myFONDO=$("#rendicontazione_tipofondo").val()
  myENTE=$("#rendicontazione_ente").val()
  myRUP=$("#flussofinanziario_rup").val()
  myPARAMS='_target='+myTARGET+"&_anno="+myANNO+"&_fondo="+myFONDO+"&_ente="+myENTE+"&_rup="+myRUP
  window.open('flussofinanziari-xls?'+myPARAMS,'_blank')
}

function printRENDICONTAZIONE(myFLUSSO)
{
  window.open('flussofinanziario-pdf?_RENDICONTAZIONE='+myFLUSSO,'_blank')
}

function openSCADENZE(myFLUSSO)
{

    
    $("#tblSCADENZE > tbody").empty();

    var page="rebuilding_action.php";
    var params="_action=getSCADENZE&_RENDICONTAZIONE="+myFLUSSO;
    $.ajax({
      type: "POST",
      url: page,
      data: params, 
      ...
      success: function(result)
      {
        ...
        {
          var aDATA=val.scadenza_data.split("-");
          var new_data=aDATA[2]+"/"+aDATA[1]+"/"+aDATA[0]
          var newRow = "<tr><td>"+val.scadenza_testo+"</td><td>"+new_data+"</td></tr>";
          $("#tblSCADENZE").append(newRow);
        });         
  ...
  $("#modalSCADENZE").modal('show')

}

function closeSCADENZE()
{


  $("#modalSCADENZE").modal('hide')
