...
class rebuildingFLUSSOFINANZIARIO {
	...
	public function __construct($idrebuilding_flussofinanziario=null) 
	{
		...
		if(!empty($idrebuilding_flussofinanziario))
		{
			...
			foreach ($aNEWS as $key => $rows) 
			{
				...
				$this->flussofinanziario_anno=$rows["flussofinanziario_anno"];
				$this->flussofinanziario_ente=$rows["flussofinanziario_ente"];
				$this->flussofinanziario_tipofondo=$rows["flussofinanziario_tipofondo"];											

				$this->flussofinanziario_areaintervento=$rows["flussofinanziario_areaintervento"];
				$this->flussofinanziario_leggeriferimento=$rows["flussofinanziario_leggeriferimento"];
				$this->flussofinanziario_titolo=$rows["flussofinanziario_titolo"];
				$this->flussofinanziario_testo=$rows["flussofinanziario_testo"];
				$this->flussofinanziario_contatti_rup=$rows["flussofinanziario_contatti_rup"];
				$this->flussofinanziario_rup=$rows["flussofinanziario_rup"];
				$this->flussofinanziario_operatore=$rows["flussofinanziario_operatore"];
				$this->flussofinanziario_datainserimento=$rows["flussofinanziario_datainserimento"];
				$this->flussofinanziario_ultimamodifica=$rows["flussofinanziario_ultimamodifica"];
				$this->flussofinanziario_stato=$rows["flussofinanziario_stato"];
				...
				$this->flussofinanziario_dtipoocumento5=$rows["flussofinanziario_tipodocumento5"];
	...
	public function insertFLUSSO($flussofinanziario_anno,$flussofinanziario_ente,$flussofinanziario_tipofondo,$flussofinanziario_areaintervento,$flussofinanziario_leggeriferimento,$flussofinanziario_areaintervento,$flussofinanziario_titolo,$flussofinanziario_testo,$flussofinanziario_documento1,$flussofinanziario_documento2,$flussofinanziario_documento3,$flussofinanziario_documento4)
	{
		global $db;
	...
	public function __construct($idrebuilding_notifica=null) 
	{
		...
		if(!empty($idrebuilding_notifica))
		{
			...
			foreach ($aNEWS as $key => $rows) 
			{
				...
				$this->notifica_oggetto=$rows["notifica_oggetto"];
				$this->notifica_testo=$rows["notifica_testo"];
				$this->notifica_destinatario=$rows["notifica_destinatario"];
				$this->idrebuilding_flussofinanziario=$rows["idrebuilding_flussofinanziario"];												
				$this->notifica_mittente=$rows["notifica_mittente"];											
	...
	public function __construct($idrebuilding_scadenzario=null) 
	{
		...
		if(!empty($idrebuilding_scadenzario))
		{
			...
			foreach ($aNEWS as $key => $rows) 
			{
				...
				$this->scadenza_testo=$rows["scadenza_testo"];
				$this->scadenza_destinatario=$rows["scadenza_destinatario"];
				$this->idrebuilding_flussofinanziario=$rows["idrebuilding_flussofinanziario"];												
	...
	public function __construct($idrebuilding_rendicontazione=null) 
	{
		...
		if(!empty($idrebuilding_rendicontazione))
		{
			...
			foreach ($aRENDICONTAZIONE as $key => $rows) 
			{
				...
				$this->idrebuilding_flussofinanziario=$rows["idrebuilding_flussofinanziario"];
				$this->rendicontazione_ente=$rows["rendicontazione_ente"];
				$this->rendicontazione_titolo=$rows["rendicontazione_titolo"];												
