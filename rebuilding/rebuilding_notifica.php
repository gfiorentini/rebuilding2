...
if(getPARAMETRO("_salva"))
{
  ...
  $notifica_documentotime=date("Ymdhis");


  //print_r_formatted($_REQUEST);

  for($i=1;$i<=$nALLEGATI;$i++)
  {    

    $idrebuilding_notifica_documento=getPARAMETRO("_notifica_documento".$i);
    $idrebuilding_notifica_documento=$db->escape_text($idrebuilding_notifica_documento);

    $documentoNAME = basename($_FILES["nome_file".$i]["name"]); 
    if(!empty($documentoNAME) && empty($idrebuilding_notifica_documento))
    {
      $path_parts = pathinfo($_FILES["nome_file".$i]["name"]);
      $filename=md5(date("Ymdhis").$documentoNAME).".".$path_parts['extension'];

      //$target_dir.$filename; echo "<br>";
      $_FILES["nome_file".$i]["tmp_name"];
     
      copy($_FILES["nome_file".$i]["tmp_name"],$target_dir.$filename);    

      $sSQL="INSERT INTO rebuilding_notifica_documento (idrebuilding_notifica,notifica_documentonome,notifica_documentotitolo,notifica_documentodata,notifica_documentooperatore,notifica_documentoultimamodifica,notifica_documentotime) values('$pidrebuilding_notifica','$filename','$documentoNAME','$pnotifica_datainserimento','$idoperatore','$pnotifica_datainserimento','$notifica_documentotime') ";
      $db->query($sSQL);      
    }
    /*
    elseif(!empty($documentoNAME) && !empty($idrebuilding_notifica_documento))
    {
      $path_parts = pathinfo($_FILES["nome_file".$i]["name"]);
      $filename=md5(date("Ymdhis").$documentoNAME).".".$path_parts['extension'];
      copy($_FILES["nome_file".$i]["tmp_name"],$target_dir.$filename);    
      $sSQL="UPDATE rebuilding_notifica_documento set notifica_documentonome='$filename',notifica_documentooperatore='$idoperatore',notifica_documentotitolo='$documentoNAME',notifica_documentoultimamodifica='$ultimamodifica',notifica_documentotime='$notifica_documentotime' where idrebuilding_notifica_documento='$idrebuilding_notifica_documento'";

    }
    else
    {
      $ultimamodifica=date("Y-m-d");
      $sSQL="update rebuilding_notifica_documento set notifica_documentonome='$allegatoTITOLO',notifica_documentooperatore='$idoperatore',notifica_documentotitolo='$documentoNAME',notifica_documentotime='$ultimamodifica',notifica_documentotime='$notifica_documentotime' where idrebuilding_notifica_documento='$idrebuilding_notifica_documento'";
  ...
  //$sSQL="delete from rebuilding_notifica_documento where idrebuilding_notifica='$pidrebuilding_notifica' and notifica_documentotime!='$notifica_documentotime'";
  //$db->query($sSQL);
...
elseif(getPARAMETRO("_eliminaallegato") && $operatore_flagamministratore==1 && empty($operatore_flagdirigente))
{
  ...
  $pidrebuilding_notifica_documento=getPARAMETRO("_allegato");
  $pidrebuilding_notifica_documento=$db->escape_text($pidrebuilding_notifica_documento);

  $sSQL="delete from rebuilding_notifica_documento where idrebuilding_notifica_documento='$pidrebuilding_notifica_documento'";
  $db->query($sSQL);
}
...
elseif(getPARAMETRO("_elimina"))
{
  ...
  $db->query($sSQL);
  $sSQL="delete from rebuilding_notifica_documento where idrebuilding_notifica='$pidrebuilding_notifica'";
  $db->query($sSQL);
}
              ...
              <div class="card-body">
                <form id="NOTIFICAFORM" name="NOTIFICAFORM" method="post" role="form" enctype="multipart/form-data" onsubmit="return doOnSubmit()">
                  ...
                  <div class="form-group row">
                    ...
                    <label for="formENTEselect">Destinatari ATS*</label>
    ...
    <select class="selectpicker" multiple id="formSTRUTTUREselect" name="formSTRUTTUREselect" title="Strutture Regionali*" <?php echo $disabled_notifica;?>  data-actions-box="true">
      ...
      <option value="CDI">Settore disabilità</option>                              -->
                  ...
                  <div class="form-group row">
                    <div class="col-12">
                      <p align="right">
                        <button type="button" class="btn btn-primary btn-xs" onclick="addALLEGATO()" <?php echo $disabled_notifica;?> <?php if(empty($pidrebuilding_notifica)) echo "style='display:none';"?> ><i class="fe fe-plus"></i>&nbsp; Allegato</button>             
                      </p> 
                    </div>
                  </div>
                  ...
                  <div id="div_allegati">
                      ...
                      <?php
                        ...
                        if(!empty($pidrebuilding_notifica))
                        {
                          ...
                          $aDOCUMENTI=$notifica->getALLEGATI();
                          $counter_allegati=0;
                          $rigadocumento="";
                          foreach ($aDOCUMENTI as $key => $aDATI) 
                          {
                            $idrebuilding_notifica_documento=$aDATI["idrebuilding_notifica_documento"];
                            $notifica_documentonome=$aDATI["notifica_documentonome"];
                            $notifica_documentotitolo=$aDATI["notifica_documentotitolo"];
                            $counter_allegati++;
                            
                            if(empty($operatore_flagamministratore) || !empty($operatore_flagamministratore))
                            {

                              $confirm='<a href=\'#\'><span class=\'badge bg-primary-soft\'>NO</span></a>&nbsp;<a href=\'rebuilding_notifica.php?_NOTIFICA='.$pidrebuilding_notifica.'&_allegato='.$idrebuilding_notifica_documento.'&_eliminaallegato=true\'><span class=\'badge bg-primary-soft\' >SI</span></a>';
                              ...
                              $rigadocumento.='<div class="col-12 col-md-10" >
                                      <input type="text" id="allegatoTITOLO'.$counter_allegati.'" name="allegatoTITOLO'.$counter_allegati.'" class="form-control form-control-text form-control-xs"  placeholder="titolo/descrizione" value="'.$notifica_documentotitolo.'" readonly>
                                    </div>';
                              $rigadocumento.='<div class="col-12 col-md-2" >
                                      <button id="consultafile" name="consultafile" type="button" class="btn btn-xs btn-rounded-circle btn-primary" onclick="viewALLEGATO(\''.$target_dir.$notifica_documentonome.'\')" ><i class="fe fe-file"></i></button>';

                              $rigadocumento.='<button type="button" id="deletefile'.$counter_allegati.'" name="filesassegnazione'.$counter_allegati.'" class="btn btn-xs btn-rounded-circle btn-danger" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="left" title="Confermi l\'eliminazione?" data-bs-content="'.$confirm.'" data-bs-html=true data-bs-trigger="focus" '.$disabled_notifica.'><i class="fe fe-x"></i></button>';
                              
                              $rigadocumento.='</div>
                                  </div>
                                  <input type="hidden" id="_notifica_documento'.$counter_allegati.'" name="_notifica_documento'.$counter_allegati.'" value="'.$idrebuilding_notifica_documento.'">
                                  </p>';                            
                            }
...
function doOnSubmit()
{
  ...
  // GF --
  // Salva la selezione delle strutture nel campo hidden
  $("#formSTRUTTURE").val($("#formSTRUTTUREselect").val());
  //
...
function deleteALLEGATO(idallegato)
{
  ...
  idDOCUMENTO=$("#_notifica_documento"+idallegato).val()

  if(idDOCUMENTO)
  {

    var page="rebuilding_action.php";
    var params="_action=deletedocumentonotifica&_documento="+idDOCUMENTO;
    $.ajax({
      type: "POST",
      url: page,
      data: params, 
